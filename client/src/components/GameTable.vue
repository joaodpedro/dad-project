<template>
    <div class="gameTable">
        ...
        <div class="alert" :class="gameAlertType" v-if="gameAlertMessage" v-html="gameAlertMessage" role="alert"></div>
...
export default {
    ...
    methods:{
        ...
        sendGameNotification(message, alertType){
            this.gameAlertMessage = message;
            this.gameAlertType = alertType;
            setTimeout(() =>{ this.gameAlertMessage = ''; this.gameAlertType = ''; }, 3000);
        }
    },
    mounted(){
        this.getGamePlayers();
